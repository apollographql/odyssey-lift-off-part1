...
const INCREMENT_TRACK_VIEWS = gql`
  mutation IncrementTrackViews($incrementTrackVewsId: ID!) {
    incrementTrackVews(id: $incrementTrackVewsId) {
      code
      message
      success
      track {
...
const TrackCard: React.FC<{ track: any }> = ({ track }) => {
  ...
  const [incrementTrackViews, {loading, error, data}] = useMutation(INCREMENT_TRACK_VIEWS, {
    variables: {
      incrementTrackVewsId: id
    },
    onCompleted: (data) => console.log(data)
  })

  return (
    <CardContainer to={`/track/${track.id}`} onClick={() => incrementTrackViews()}>
      <CardContent>
        <CardImageContainer>
